...
Path::Path()
    : m_originalPath(String()), m_folder(String()), m_isFolderAbsolute(false),
      m_filename(String()), m_extension(String()) {}

Path::Path(const char *path)
    : m_originalPath(path), m_folder(String()), m_isFolderAbsolute(false),
      m_filename(String()), m_extension(String()) {
...
Path::Path(const String &path)
    : m_originalPath(path), m_folder(String()),
      m_isFolderAbsolute(std::make_shared<bool>()), m_filename(String()),
      m_extension(String()) {
  parseFileName(m_originalPath, m_folder, m_filename, m_extension);
  setIsFolderAbsolute();
}
...
bool Path::isFileExist() const { return isFileExist(absolutePath()); }

bool Path::isFileExist(const Path &path) {
  return isFileExist(path.absolutePath());
}

bool Path::isFileExist(const String &path) {
#ifdef _WIN32
...
bool Path::isFolderExist() const { return isFolderExist(*this); }

bool Path::isFolderExist(const Path &path) {
  return isFolderExist(path.folder());
}

bool Path::isFolderExist(const String &path) {
#ifdef _WIN32
...
void Path::parseFileName(const String &path, String &folder, String &filename,
                         String &extension) {
...
String Path::relativePath() const { return relativePath(*this, currentDir()); }

String Path::relativePath(const String &relativeTo) const {
  return relativePath(*this, relativeTo);
}

String Path::relativePath(const Path &path, const String &relativeTo) {
...
String Path::absoluteFolder(const Path &path) {
...
String Path::toUnixFormat(const String &path) {
  String pathOut(path);
...
String Path::toWindowsFormat(const String &path) {
  String pathOut(path);
  size_t pos(pathOut.rfind("/"));
  while (pos != std::string::npos) {
    pathOut.replace(pos, 1, "\\\\");
...
String Path::originalPath() const { return m_originalPath; }

String Path::folder() const { return m_folder; }

void Path::setFilename(const String &name) { m_filename = name; }

String Path::filename() const { return m_filename; }

void Path::setExtension(const String &ext) { m_extension = ext; }

String Path::extension() const { return m_extension; }
...
void Path::setIsFolderAbsolute() {
  ...
  std::smatch matches;
  m_isFolderAbsolute =
      std::regex_search(current, matches, std::regex("^([A-Z]):[\\/].*$"));
#else
  base = "/";
...
void Path::createFolder() const {
  ...
  do {
    ...
    if (sep != std::string::npos) {
      ...
      // Séparer la première et la dernière partie
      if (!isFolderExist(static_cast<String>(tp.substr(0, sepTrack)))) {
#ifdef _WIN32
