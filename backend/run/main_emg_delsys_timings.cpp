...
public:
  ...
  void stop_recording() { recording_file_.close(); }

  void stop() { send_cmd("STOP"); }
...
protected:
  void initialize() {
    asio::ip::tcp::endpoint comm_endpoint(
        asio::ip::address::from_string("127.0.0.1"), cmd_port_);
    asio::ip::tcp::endpoint data_endpoint(
        asio::ip::address::from_string("127.0.0.1"), data_port_);
...
public:
  TrignoEMG(const std::pair<int, int> &channel_range, int samples_per_read,
      ...
      : BaseTrignoDaq(host, cmd_port, data_port, 16, timeout),
...
public:
  TrignoAUX(const std::pair<int, int> &channel_range, int samples_per_read,
            const std::string &units = "V",
            const std::string &host = "localhost", int cmd_port = 50040,
            int data_port = 50044, double timeout = 10)
      : BaseTrignoDaq(host, cmd_port, data_port, 144, timeout),
        channel_range_(channel_range), samples_per_read_(samples_per_read),
        scaler_(1.0) {
    if (units == "mV") {
      scaler_ = 1000.0;
...
int main() {
  try {
    int runnumber;
    std::cout << "Enter how much seconds you want the trial to last : "
              << std::endl;
    std::cin >> runnumber;
    ...
    emg.start(); // only fire START command once all Trigno instances have been
                 // summonned and recording have been launched
    ...
    std::thread t1firstrun([&emg]() {
      ...
      auto durationt1firstrun =
          std::chrono::duration_cast<std::chrono::microseconds>(
              endt1firstrun - startt1firstrun);
      std::cout << "EMG data time for 2000hz : " << durationt1firstrun.count()
                << std::endl;
      std::cout << "Diff on 1s : " << durationt1firstrun.count() - 1000000
    ...
    std::thread t2firstrun([&aux]() {
      ...
      auto durationt2firstrun =
          std::chrono::duration_cast<std::chrono::microseconds>(
              endt2firstrun - startt2firstrun);
      std::cout << "AUX data time for 148.148hz : "
                << durationt2firstrun.count() << std::endl;
      std::cout << "Diff on 1s : " << durationt2firstrun.count() - 1000000
                << " microseconds" << std::endl;
    });
    t1firstrun.join();
    t2firstrun.join();
    ...
    auto durationtotalfirstrun =
        std::chrono::duration_cast<std::chrono::microseconds>(
            endtotalfirstrun - starttotalfirstrun);
    std::cout << "Total data time for 1s, first run : "
              << durationtotalfirstrun.count() << std::endl;
    std::cout << "Diff on 1s, first run : "
              << durationtotalfirstrun.count() - 1000000 << " microseconds"
              << std::endl;
    std::cout << std::endl;
    std::cout << std::endl;
    ...
    for (size_t i = 0; i < runnumber; i++) {
      ...
      std::thread t1([&emg]() {
        ...
        auto durationt1 = std::chrono::duration_cast<std::chrono::microseconds>(
            endt1 - startt1);
        std::cout << "EMG data time for 2000hz : " << durationt1.count()
                  << std::endl;
        std::cout << "Diff on 1s : " << durationt1.count() - 1000000
                  << " microseconds" << std::endl;
      });
      std::thread t2([&aux]() {
        ...
        auto durationt2 = std::chrono::duration_cast<std::chrono::microseconds>(
            endt2 - startt2);
        std::cout << "AUX data time for 148.148hz : " << durationt2.count()
                  << std::endl;
        std::cout << "Diff on 1s : " << durationt2.count() - 1000000
                  << " microseconds" << std::endl;
      });
      t1.join();
      t2.join();
    ...
    std::cout << std::endl;
    std::cout << std::endl;
    std::cout << "Total data time for " << runnumber
              << "s : " << durationtotal.count() << std::endl;
    std::cout << "Diff on " << runnumber
              << "s : " << durationtotal.count() - (1000000 * runnumber)
              << " microseconds" << std::endl;
    emg.stop();
    emg.stop_recording();
    aux.stop_recording();
