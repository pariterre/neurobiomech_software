...
enum PredictionsGraphType { analyses }

class PredictionsGraphController {
  Predictions _data;
  final PredictionsGraphType graphType;

  Function()? _updateCallback;

  set data(Predictions data) {
    _data = data;
    if (_updateCallback != null) _updateCallback!();
  }

  PredictionsGraphController(
      {required Predictions data, required this.graphType})
      : _data = data;
}

class PredictionsGraph extends StatefulWidget {
  const PredictionsGraph(
      {super.key, required this.controller, this.combineGraphs = false});

  final bool combineGraphs;
  final PredictionsGraphController controller;

  @override
  State<PredictionsGraph> createState() => _PredictionsGraphState();
}

class _PredictionsGraphState extends State<PredictionsGraph> {
  @override
  void initState() {
    super.initState();
  ...
  Iterable<LineChartBarData?> _dataToLineBarsData({int? prediction}) {
    final timeSeries = _predictions;
  ...
  List<double> get _predictions {
    switch (widget.controller.graphType) {
      case PredictionsGraphType.analyses:
        return widget.controller._data.predictions;
    }
  }

  int get _predictionCount => _predictions.length;

  bool _combinePredictions = true;
  void _onChanged(bool combinePredictions) {
    setState(() {
      _combinePredictions = combinePredictions;
    });
  }

  late final List<bool> _showPredictions =
      List.generate(_predictionCount, (_) => true);
  void _onPredictionSelected(int prediction, bool newValue) {
    setState(() {
      _showPredictions[prediction] = newValue;
    });
  }
